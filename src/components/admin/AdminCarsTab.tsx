...
interface AdminCarsTabProps {
  cars: CarType[];
  onRefresh: () => void;
  showToast: (msg: string, type?: 'success' | 'error') => void;
}

export const AdminCarsTab: React.FC<AdminCarsTabProps> = ({ cars, onRefresh, showToast }) => {
  ...
  const [carForm, setCarForm] = useState({
    ...
    model: '',
    registration_number: '',
    category: 'SUV' as 'Sedan' | 'SUV' | 'Luxury' | 'Hatchback' | 'MUV' | 'Van',
    seating_capacity: 7,
  ...
  const filteredCars = cars.filter(c => {
    ...
    const matchesStatus =
      carStatusFilter === 'all' ||
      (carStatusFilter === 'available' && (c.status === 'available' || !c.status)) ||
      c.status === carStatusFilter;

    const matchesCategory =
      carCategoryFilter === 'all' || c.category.toLowerCase() === carCategoryFilter.toLowerCase();
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-3 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-lg font-black text-cyan-400">
              {cars.reduce((acc, c) => acc + (c.available_slots ?? (c.total_slots || 5)), 0)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/[0.03] backdrop-blur-xl p-4 rounded-2xl border border-white/10 flex flex-wrap items-center justify-between gap-3">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            placeholder="Search by car model, brand, plate number..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredCars.map(c => {
          ...
          const coverImg = Array.isArray(c.images) && c.images[0] ? c.images[0] : 'https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?auto=format&fit=crop&w=1200&q=80';
              ...
              <div>
                ...
                <div className="relative h-44 w-full overflow-hidden group bg-slate-950">
                  ...
                  <div className="absolute top-3 left-3 flex gap-1.5">
                    ...
                    {c.registration_number && (
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-bold bg-slate-900/90 text-white border border-white/20">
                        {c.registration_number}
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className="absolute bottom-3 right-3 px-2.5 py-1 bg-slate-950/80 hover:bg-slate-900 text-slate-200 hover:text-amber-400 rounded-xl text-[10px] font-bold border border-white/20 flex items-center gap-1 backdrop-blur-md transition cursor-pointer"
                  >
                    <ImageIcon className="w-3 h-3 text-amber-400" />
                    <span>{c.images?.length || 1} Photos</span>
                  </button>
                </div>
                ...
                <div className="p-4 space-y-3">
                  <div>
                    ...
                    <div className="text-base font-black text-white">{c.name}</div>
                  </div>

                  <div className="flex items-baseline justify-between pt-1 border-t border-white/5">
                    <div>
                      <span className="text-lg font-black text-amber-400">₹{c.price_per_day}</span>
                  ...
                  {/* Slot Allocation Box */}
                  <div className="p-2.5 bg-slate-950/60 rounded-xl border border-white/5 flex items-center justify-between">
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase">Active Slots</div>
          ...
          <div className="bg-slate-900 border border-white/10 max-w-3xl w-full rounded-3xl p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveCar} className="space-y-4 text-xs">
              ...
              <div className="p-3.5 bg-white/[0.02] border border-amber-500/20 rounded-2xl space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block text-slate-300 mb-1">Total Fleet Slots</label>
                    <input
                      type="number"
                      min="1"
                  ...
                  <div>
                    <label className="block text-slate-300 mb-1">Currently Available Slots</label>
                    <input
                      type="number"
                      min="0"
                      max={carForm.total_slots || 5}
                      value={carForm.available_slots ?? 4}
                  ...
                  <div>
                    <label className="block text-slate-300 mb-1">Operational Status</label>
                    <select
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block font-bold text-slate-300 mb-1">Daily Rental Rate (₹) *</label>
                  <input
                    type="number"
                    required
                ...
                <div>
                  <label className="block font-bold text-slate-300 mb-1">Pre-booking Slot Lock (₹)</label>
                  <input
                    type="number"
                    required
                ...
                <div>
                  <label className="block font-bold text-slate-300 mb-1">Refundable Security Deposit (₹)</label>
                  <input
                    type="number"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-300 mb-1">Depot / Pickup Hub Location</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block font-bold text-slate-300 mb-1">Key Vehicle Features (Comma separated)</label>
                  <input
                    type="text"
                    value={carForm.features || ''}
                    onChange={e => setCarForm({ ...carForm, features: e.target.value })}
                    placeholder="7 Captain Seats, Sunroof, Dual AC, Cruise Control"
                    className="w-full p-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:ring-2 focus:ring-amber-400 focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="block font-bold text-slate-300">Vehicle Photos (URLs)</label>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {carForm.imagesList.map((img, idx) => (
                    <div key={idx} className="relative group w-20 h-14 rounded-lg overflow-hidden border border-white/20">
                      <img src={img} alt="Vehicle thumbnail" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      <button
                        type="button"
                        onClick={() => {
                          setCarForm({
                            ...carForm,
                            imagesList: carForm.imagesList.filter((_, i) => i !== idx),
                          });
                        }}
